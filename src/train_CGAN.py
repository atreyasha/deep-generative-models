...
def trainCGAN(data, learning_rate, epochs, batch_size, im_dim, num_filters,
    ...
    current_time = getCurrentTime() + "_" + re.sub(
        ",", "_", str(latent_dimensions)) + "_" + data + "_cgan"
    os.makedirs("pickles/" + current_time)
    # create model
    model = CGAN(latent_dim=latent_dimensions,
                 epochs=epochs,
    ...
    fieldnames = [
        ...
        "num_filters", "latent_dimensions", "g_factor", "drop_rate"
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="mnist",
        help=
        "data source to train CGAN, possibilities are 'mnist', 'fashion_mnist' and 'faces' <default: 'mnist'>"
    )
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        " scalar multiple by which learning rate for the generator is multiplied <default: 1>"
    )
    parser.add_argument(
        "--drop-rate",
        type=float,
    ...
    requiredNamed.add_argument(
        ...
        type=int,
        help=
        "number of central latent dimensions in CGAN, 2 dimensions are recommended for quick manifold visualization",
        required=True)
    args = parser.parse_args()
    # train CGAN based on parameters
    trainCGAN(args.data, args.learning_rate, args.epochs, args.batch_size,
              args.im_dim, args.num_filters, args.latent_dimensions,
              args.g_factor, args.drop_rate)
