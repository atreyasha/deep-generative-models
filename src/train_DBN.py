...
def trainDBN(data, learning_rate, k1, k2, epochs, batch_size, dims):
    ...
    if data == "mnist" or data == "fashion_mnist":
        ...
        x_train = [
            tf.cast(tf.reshape(x, shape=(784, 1)), "float32") for x in x_train
        ]
    elif data == "faces":
    ...
    current_time = getCurrentTime() + "_" + re.sub(",", "_",
                                                   dims) + "_" + data + "_dbn"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="mnist",
        help=
        "data source to train DBN, possibilities are 'mnist', 'fashion_mnist' and 'faces' <default: 'mnist'>"
    )
    ...
    parser.add_argument(
        ...
        default=5,
        help=
        "number of Gibbs-sampling steps during PCD-k algorithm <default: 5>")
    parser.add_argument(
        "--epochs",
        type=int,
        default=1,
    ...
    requiredNamed.add_argument(
        ...
        type=str,
        help=
        "consecutive enumeration of visible and hidden layers separated by a comma character, eg. 784,500,784,500",
        required=True)
    args = parser.parse_args()
    # train DBN based on parameters
    trainDBN(args.data, args.learning_rate, args.k1, args.k2, args.epochs,
             args.batch_size, args.dimensions)
