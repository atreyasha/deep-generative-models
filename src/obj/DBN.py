...
class DBN:
    ...
    def train_PCD(self, data):
        ...
        for i in range(len(self.models)):
            ...
            if i != len(self.models) - 1:
                ...
                data = [
                    self.models[i].random_sample(self.models[i].prop_up(img))
                    for img in data
                ]
            else:
    ...
    def generate_visible_samples(self,
                                 k=15,
                                 number_samples=100,
                                 indices=None,
                                 mean_field=True):
        ...
        if indices is None:
            new_data = [
                self.models[len(self.models) - 1].gibbs_sampling(img, k)
                for img in sample(samples, number_samples)
            ]
        else:
        ...
        for i in reversed(range(len(self.models) - 1)):
            ...
            if mean_field:
                ...
                new_data = [
                    self.models[i].random_sample(self.models[i].prop_down(img))
                    for img in new_data
                ]
        return new_data
