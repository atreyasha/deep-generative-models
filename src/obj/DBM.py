...
class DBM:
    ...
    def train_PCD(self, data):
        ...
        for i in range(len(self.models)):
            ...
            if i == 0:
                print("Bottom RBM")
                self.models[i].persistive_contrastive_divergence_k(
            ...
            if i != len(self.models) - 1:
                ...
                if self.models[i + 1].w.get_shape().as_list() == tf.transpose(
                        self.models[i].w).get_shape().as_list():
                    print(
                        "Assigning previously learned transpose-weights to next model"
                    )
                    self.models[i + 1].w = tf.Variable(
                        tf.transpose(self.models[i].w))
                    self.models[i + 1].b_h = tf.Variable(self.models[i].b_v)
            data = [
                self.models[i].random_sample(self.models[i].prop_up(img, q=2))
                for img in data
            ]
            self.data.append(data)
                ...
                for i in range(len(self.models)):
                    if i != len(self.models) - 1:
                        data = [
                            tf.nn.sigmoid(
                                tf.add(
                                    self.models[i].prop_up(data[j], sig=False),
                                    self.models[i + 1].prop_down(
                                        self.data[i + 2][indices[j]],
                                        sig=False))) for j in range(len(data))
                        ]
                    else:
                        data = [
                            self.models[i].prop_up(data[j])
                ...
                for i in reversed(range(len(self.models))):
                    if i != 0:
                        data = [
                            tf.nn.sigmoid(
                                tf.add(
                                    self.models[i].prop_down(data[j],
                                                             sig=False),
                                    self.models[i - 1].prop_up(
                                        self.data[i - 1][indices[j]],
                                        sig=False))) for j in range(len(data))
                        ]
                    else:
                        data = [
