...
class RBM:
    ...
    def __init__(self,
        ...
        self.w = tf.Variable(tf.random.uniform(shape=(num_hidden, num_visible),
                                               maxval=1,
                                               minval=-1),
                             dtype="float32")
        # bias over hidden variables
        self.b_h = tf.Variable(tf.random.uniform(shape=(num_hidden, 1),
                                                 maxval=1,
                                                 minval=-1),
                               dtype="float32")
        # bias over visible variables
        self.b_v = tf.Variable(tf.random.uniform(shape=(num_visible, 1),
                                                 maxval=1,
                                                 minval=-1),
                               dtype="float32")
    ...
    def random_sample(self, input_v):
        ...
        return tf.where(
            tf.random.uniform(shape=tf.shape(input_v)) - input_v < 0,
    ...
    def prop_down(self, h, q=1, sig=True):
        ...
        if sig:
            return tf.nn.sigmoid(
                q * tf.add(self.b_v, tf.matmul(tf.transpose(self.w), h)))
        else:
        ...
        for i in range(self.epochs):
            ...
            for batch in tensors:
                if j % 20 == 0:
                    ...
                    if j != 0:
                        tf.print(
                            "Mean gradient 2-norm:",
                            tf.reduce_mean([log_g / j, log_h / j, log_v / j]))
                ...
                if position is None:
                    ...
                    g = tf.reduce_mean(
                        tf.stack([
                            tf.matmul(u[i], tf.transpose(batch[i]))
                            for i in range(self.batch_size)
                        ]), 0)
                    # compute sampled gibbs
                    v_new = tf.map_fn(
                        lambda x: self.gibbs_sampling(x, self.k2), batch)
                    u_new = tf.map_fn(self.prop_up, v_new)
                    # compute change to gradient, average gradient shifts before adding
                    g_delta = -1 * tf.reduce_mean(
                        tf.stack([
                            tf.matmul(u_new[i], tf.transpose(v_new[i]))
                            for i in range(self.batch_size)
                        ]), 0)
                    ...
                    # compute sampled gibbs
                    v_new = tf.map_fn(
                        lambda x: self.gibbs_sampling(x, self.k2, q1=2, q2=1),
                        batch)
                    ...
                    # compute sampled gibbs
                    v_new = tf.map_fn(
                        lambda x: self.gibbs_sampling(x, self.k2, q1=1, q2=2),
                        batch)
        ...
        for i in range(self.epochs):
            ...
            for batch in tensors:
                ...
                if position is None:
                    ...
                    if j == 0:
                        v_new = tf.map_fn(
                            lambda x: self.gibbs_sampling(x, self.k1), batch)
                    v_new = tf.map_fn(
                        lambda x: self.gibbs_sampling(x, self.k2), v_new)
                    u_new = tf.map_fn(self.prop_up, v_new)
                    # compute change to gradient, average gradient shifts before adding
                    g_delta = -1 * tf.reduce_mean(
                        tf.stack([
                    ...
                    if j == 0:
                        v_new = tf.map_fn(
                            lambda x: self.gibbs_sampling(
                                x, self.k1, q1=2, q2=1), batch)
                    v_new = tf.map_fn(
                        lambda x: self.gibbs_sampling(x, self.k2, q1=2, q2=1),
                        v_new)
                    u_new = tf.map_fn(lambda x: self.prop_up(x, q=2), v_new)
