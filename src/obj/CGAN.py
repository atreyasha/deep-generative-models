...
class CGAN(tf.keras.Model):
    def __init__(self,
        ...
        self.optimizer_g = tf.keras.optimizers.Adam(self.learning_rate *
                                                    self.g_factor)
        ...
        self.generative_net = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(units=(int(
                (self.im_dim / 2)**2) * self.n_filters),
                                  activation="relu",
                                  name="g1"),
            tf.keras.layers.Reshape(target_shape=(int(self.im_dim / 2),
                                                  int((self.im_dim / 2)),
                                                  self.n_filters)),
            tf.keras.layers.Conv2DTranspose(filters=int(self.n_filters),
                                            kernel_size=3,
                                            strides=(2, 2),
                                            padding="SAME",
                                            activation="relu",
        ...
        self.discriminator_net = tf.keras.Sequential([
            tf.keras.layers.InputLayer(input_shape=(int(self.im_dim),
                                                    int(self.im_dim), 1)),
            tf.keras.layers.Conv2D(filters=int(self.n_filters),
                                   kernel_size=3,
                                   strides=(2, 2),
            ...
            tf.keras.layers.Dropout(self.drop_rate),
            tf.keras.layers.Dense(units=int((self.im_dim / 4)**2),
                                  activation=tf.nn.leaky_relu,
                                  name="d3"),
            tf.keras.layers.Dropout(self.drop_rate),
    ...
    def compute_loss_discriminator(self, x):
        ...
        cross_ent_x = tf.nn.sigmoid_cross_entropy_with_logits(
            logits=x_logit, labels=tf.ones(shape=x_logit.shape))
        cross_ent_gen = tf.nn.sigmoid_cross_entropy_with_logits(
            logits=-gen_logit, labels=tf.ones(shape=gen_logit.shape))
    ...
    def train(self, train_dataset):
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices(
            train_dataset).shuffle(train_dataset.shape[0]).batch(
                self.batch_size)
        for i in range(self.epochs):
            ...
            for train_x in train_dataset:
                # disciminator step
                gradients, t_v, loss = self.compute_gradients(
                    train_x, sub="discriminator")
                Loss_d += loss
                norm_d += tf.reduce_mean([tf.norm(g) for g in gradients])
                self.apply_gradients(gradients, t_v, sub="discriminator")
                # generator step
                gradients, t_v, loss = self.compute_gradients(train_x,
                                                              sub="generator")
                Loss_g += loss
                ...
                if j != 1 and j % 20 == 0:
                    ...
                    tf.print("Epoch: %s, Batch: %s/%s" %
                             (i + 1, j, num_samples))
                    tf.print("Mean discriminator loss: ", Loss_d / j,
                             ", Mean discriminator gradient norm: ",
                             norm_d / j)
                    tf.print("Mean generator loss: ", Loss_g / j,
